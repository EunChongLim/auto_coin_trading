...
- `1` 입력: 기본 테스트 (최근 1, 2, 3, 7일)
- `2` 입력: 확장 테스트 (최근 1, 2, 3, 7, 14, 30일)
- `3` 입력: RSI 85 테스트 (RSI 매도 기준 상향 비교)

**여러 날짜를 한 번에 테스트**하여 알고리즘의 안정성을 검증합니다!
...
**💡 팁**: 여러 날짜로 테스트하여 알고리즘의 **안정성과 일관성**을 검증하세요!

---

## 📈 실시간 거래 출력 예시 (v2.0 비트코인 최적화)
...
- ✅ **RSI 계산 방식**: SMA → **EMA 기반** (최신 데이터 가중, 빠른 반응)
...
- ✅ **백테스팅 기능**: 과거 데이터로 알고리즘 검증 (`backtest.py`)
